...
// Definicija strukture za korisniËki profil
typedef struct {
    char ime[100];
    char prezime[100];
    int bodovi;
...
// Definicija Ëvora u binarnom stablu
...
TreeNode* createNode(UserProfile data) {
    ...
    if (newNode == NULL) {
        ...
        exit(1);  // Izaite iz programa ako malloc ne uspije
    }
    newNode->data = data;
    newNode->left = NULL;
    newNode->right = NULL;
...
// Funkcija za umetanje Ëvora u binarno stablo
...
// Funkcija za Ëitanje podataka iz datoteke i izgradnju stabla
void readFileAndBuildTree(const char* filename, TreeNode** root) {
    ...
    char temp[200];  // Privremena varijabla za Ëitanje svake linije iz datoteke
    ...
    while (fgets(temp, sizeof(temp), file)) {
        ...
        if (strncmp(temp, "Uspjeh u bodovima:", 18) == 0) {
            ...
            // Umetnemo profil u stablo pomoÊu funkcije insertNode
...
void rima_test(const char* fileName) {
    FILE* file = fopen(fileName, "r");
    if (!file) {
        printf("Greöka pri otvaranju datoteke: %s\n", fileName);
        return;
    }

    char line[100];

    // »itanje i ispisivanje sadrûaja datoteke
...
void slogovi_test(const char* fileName) {
    ...
    // »itanje zadataka iz datoteke i ispis
    while (fgets(line, sizeof(line), file)) {
        printf("%s", line);
    }
...
void fonemi_test(const char* fileName) {
    ...
    // »itanje i ispisivanje svih zadataka iz datoteke
    while (fgets(line, sizeof(line), file)) {
        printf("%s", line);
    }
...
int unos_bodova() {
    ...
    for (int i = 0; unos[i] != '\0'; i++) {
        ...
        else if (unos[i] == '+') {  // Ako naiemo na '+', dodajemo broj u bodove
...
// Funkcija za ispisivanje sortiranog popisa iz datoteke i izvoûenje u drugu datoteku
...
position kreiranje_profila() {
    ...
    if (newPerson) {
        ...
        if (file == NULL) {
            printf("Greöka pri otvaranju datoteke za pisanje.\n");
            return NULL;
        }
        ...
        printf("Greöka pri unosu profila.\n");
        return NULL;
    }
    return newPerson;
}
...
int vjezba_rime() {
    ...
    while (1) { // Petlja za rimu
        ...
        if (bod < 8) {
            ...
            printf("\nNa ovom zadatku nije moguÊe ostvariti navedeni broj bodova. Pokuöajte ponovno.\n");
        }
        brojac++;
    }
}
    ...
    while (1) {
        ...
        if (opcija == 1) {
            do {
                ...
                printf("\néelite li ispitati joö osoba? (da/ne): ");
                fgets(izbor, sizeof(izbor), stdin);
                izbor[strcspn(izbor, "\n")] = '\0';
